...
def edit_window(evt, files=None):
	...
	if files:
		...
		new_stack_name = entry_name.get()
		ok_button.bind('<Button-1>', lambda evt,args=(True, new_stack_name):stack.new_stack_file(evt,*args))
...
stack_browser.bind('<<ListboxSelect>>', lambda evt, arg=files:selectlistbox(evt, arg))
stack_browser.bind('<Double-1>', lambda evt, arg=files:edit_window(evt, arg))
card_browser.bind('<Double-1>', lambda evt, arg=files:edit_card(evt, arg))
